defmodule Kerosene.HTML do
  ...
  Path can be overridden by adding setting `:path` in the `opts`.
  For example:

      Kerosene.HTML.paginate(@conn, @kerosene, path: product_path(@conn, :index, foo: "bar"))

  Additional pagination class can be added by adding setting `:class` in the `opts`.
  For example:

      Kerosene.HTML.paginate(@conn, @kerosene, theme: :bootstrap4, class: "paginate-sm")
  """
